...
NUM_QUESTION_PER_QUIZ = 5
QUESTIONS_PATH = pathlib.Path(__file__).parent / "question.toml"

def prepare_questions(path, num_question):
    ...
    question = topics[topic_label]
    num_questions = min(NUM_QUESTION_PER_QUIZ, len(question))
    return random.sample(question, k=num_questions)
...
def run_quiz():
    questions = prepare_questions(
        QUESTIONS_PATH, num_question=NUM_QUESTION_PER_QUIZ
    )
    ...
    print(f"You got {num_correct} correct out of {num} questins! ")
